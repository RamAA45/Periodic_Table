/* ──────────────────────────────────────────────
   (NEW) 0 ▸  Grab summary CSVs  ➜  data[bcc|fcc|hcp][Metal] = {ICOHP,…}
   ────────────────────────────────────────────── */
const structures = ['bcc','fcc','hcp'];
const data       = {};   // filled asynchronously below

Promise.all(
  structures.map(s => fetch(`plots/${s}_summary.csv`).then(r=>r.text()))
).then(results => {
  /**
   * Convert each CSV text into an object mapping Metal → {ICOHP, ICOOP, ICOBI}
   */
  results.forEach((csvText, idx) => {
    ...
    lines.forEach(line=>{
      ...
      data[struct][Metal.trim()] = { ICOHP, ICOOP, ICOBI };
    });
  });
}).catch(console.error);
...
/* ──────────────────────────────────────────────
   4 ▸  Build ONLY the real element cells  (ADD hover events!)
   ────────────────────────────────────────────── */
const tbl = document.getElementById('periodic-table');
const infoCard = document.getElementById('info-card');   // <<–– new handle
...
/* ──────────────────────────────────────────────
   4-bis ▸  Tooltip helpers
   ────────────────────────────────────────────── */
function showInfoCard(sym){
  // guard: data may still be downloading
  if (!data.bcc) return;

  const rowsHTML = structures.map(s=>{
      const rec   = data[s][sym] || {};
      const val = x => (x ?? '—');          // graceful dash if missing
